...
void saveConfig(){
  ...
  EEPROM.commit();
  EEPROM.end();
  Serial.println("Configurazione salvata nell'EEPROM");
}
...
void loadConfig(){
  ...
  Serial.println("Configurazione caricata dall'EEPROM");
  for(int i = 0; i < 5; i++){
    Serial.print("Altcoin "); Serial.print(i); Serial.print(": "); Serial.println(customAltcoins[i]);
    Serial.print("Ticker "); Serial.print(i); Serial.print(": "); Serial.println(customAltcoinTickers[i]);
  }
}
...
void initDefaultConfigIfNeeded(){
  ...
  if(first == 0xFF || first == 0){
    Serial.println("EEPROM vuota o resettata, salvo valori predefiniti");
    customAltcoins[0] = "ethereum";
    customAltcoins[1] = "ripple";
    customAltcoins[2] = "kaspa";
    customAltcoins[3] = "dogecoin";
    customAltcoins[4] = "cardano";
    customAltcoinTickers[0] = "ETH";
    customAltcoinTickers[1] = "XRP";
    customAltcoinTickers[2] = "KAS";
    customAltcoinTickers[3] = "DOGE";
    customAltcoinTickers[4] = "ADA";
    saveConfig();
  } else {
    Serial.println("EEPROM già inizializzata");
  }
}
...
void setupWiFi(){
  ...
  // Imposta i valori per il portale
  ...
  // Ripristina i valori predefiniti se i campi sono vuoti
  for(int i = 0; i < 5; i++){
    if(customAltcoins[i].length() == 0 || customAltcoinTickers[i].length() == 0){
      customAltcoins[i] = (i == 0) ? "ethereum" : (i == 1) ? "ripple" : (i == 2) ? "kaspa" : (i == 3) ? "dogecoin" : "cardano";
      customAltcoinTickers[i] = (i == 0) ? "ETH" : (i == 1) ? "XRP" : (i == 2) ? "KAS" : (i == 3) ? "DOGE" : "ADA";
    }
  }

  Serial.println("Valori dopo WiFiManager:");
  for(int i = 0; i < 5; i++){
    Serial.print("Altcoin "); Serial.print(i); Serial.print(": "); Serial.println(customAltcoins[i]);
    Serial.print("Ticker "); Serial.print(i); Serial.print(": "); Serial.println(customAltcoinTickers[i]);
  }
